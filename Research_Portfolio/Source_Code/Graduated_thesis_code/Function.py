...
def Create_variable(model): 
  ...
  #! Biến DG
  List_DG_variable_real = {'P_DG', 'Q_DG', 'On_cost_DG'}
...
def object_function(model, export_limits, stage='Normal'):
    ...
    for time in model.time:
      for node in model.node_has_DG:
        profit2_form_DG += model.alpha_DG + model.beta_DG * model.P_DG[node,time]/fix_value+ model.theta_DG * model.P_DG[node,time]/fix_value* model.P_DG[node,time]/fix_value+ model.Cold_start_cost_DG*model.On_cost_DG[node,time]+ model.keta_DG * model.P_DG[node,time]/fix_value
    ...
    if stage == 'Normal':
        ...
        C_shed_critical = 250000 * 100
    elif stage == 'Emergency':
        # C_shed ban đầu là 1000, nhưng do chưa nhân với hệ số chia (1/fix_value = 100)
        # nên bị rẻ hơn mua điện từ lưới (18 * 100 = 1800). Tăng lên 100000 (1000 cents/kWh * 100)
        C_shed_normal = 10000
        C_shed_critical = 10000 * 50
...
def active_power_DG_constraints(model): #*(4c)
  ...
  for node in model.node_has_DG:
    for time in model.time:
      model.active_power_DG.add(model.Q_DG[node, time] <= model.P_Qmax_DG * model.I_DG[node, time])
      model.active_power_DG.add(-model.P_Qmax_DG * model.I_DG[node, time] <= model.Q_DG[node, time])
  return
...
def SOC_equal_constraints(model): #*(6f)
  ...
  if SOC_init is None:
    for node in model.node_has_BESS:
      ...
      ini = model.SOC_ini_BESS
      model.SOC_equal.add(day_end == ini)
  return
...
def print_result(model,grid_index):
  ...
  return
def create_graph_separate_MG_trading(model, exportlimit, trading_line, MG_index, model_da=None):
    t_start = getattr(model, 't_start', 0)
    ...
    filename = f"imagine_of_MG{MG_index}"
    filepath = f".\\Result_data\\{filename}.png"
    plt.savefig(filepath, dpi=300, bbox_inches='tight')
    # plt.show()
    plt.close(fig)

def create_graph_separate_MG_Q(model, exportlimit, trading_line, MG_index, model_da=None):
    import pyomo.environ as pe
    import numpy as np
    import matplotlib.pyplot as plt
    import math
    ...
    filename = f"imagine_of_MG{MG_index}_Q"
    filepath = f".\\Result_data\\{filename}.png"
    plt.savefig(filepath, dpi=300, bbox_inches='tight')
    plt.close(fig)
    # Add call to generate IEEE standard graphs
    plot_ieee_graphs(model, exportlimit, trading_line, MG_index, model_da=model_da)

    return

def plot_ieee_graphs(model, exportlimit, trading_line, MG_index, model_da=None):
    import os
    ...
    trade_data = {}
    soc_data = {}
    shed_normal = [0] * total_len
    shed_critical = [0] * total_len
        ...
        for t in range(t_start):
            for node in model_da.index_of_node:
                ...
                if hasattr(model_da, 'P_shed_normal'):
                    shed_normal[t] += value(model_da.P_shed_normal[node, t])
                if hasattr(model_da, 'P_shed_critical'):
                    shed_critical[t] += value(model_da.P_shed_critical[node, t])
                elif hasattr(model_da, 'P_shed'):
                    shed_normal[t] += value(model_da.P_shed[node, t])
    ...
    for t_idx in model.time:
        ...
        for node in model.index_of_node:
            ...
            if hasattr(model, 'P_shed_normal'):
                shed_normal[t_real] += value(model.P_shed_normal[node, t_idx])
            if hasattr(model, 'P_shed_critical'):
                shed_critical[t_real] += value(model.P_shed_critical[node, t_idx])
            elif hasattr(model, 'P_shed'):
                shed_normal[t_real] += value(model.P_shed[node, t_idx])
    ...
    if plot_trade:
        ...
        fig1.tight_layout()
        fig1.savefig(f".\\Result_data\\P2P_PowerFlow_MG{MG_index}.png", dpi=300, bbox_inches='tight')
    plt.close(fig1)
    ...
    if plot_soc:
        ...
        fig2.tight_layout()
        fig2.savefig(f".\\Result_data\\BESS_SOC_MG{MG_index}.png", dpi=300, bbox_inches='tight')
    plt.close(fig2)

    # 3. Plot Load Shedding
    fig3, ax3 = plt.subplots(figsize=(8, 4))
    if any(v > 1e-4 for v in shed_normal) or any(v > 1e-4 for v in shed_critical):
        width = 0.6
        ax3.bar(x, shed_normal, width, color='#ffcc00', label='Normal Load Shedding')
        ax3.bar(x, shed_critical, width, bottom=shed_normal, color='#cc0000', label='Critical Load Shedding')
        ax3.set_title(f'Load Shedding - MG {MG_index}', fontsize=12, fontweight='bold')
        ax3.set_xlabel('Time (h)', fontsize=10)
        ax3.set_ylabel('Power Shed (MW)', fontsize=10)
        ax3.grid(True, axis='y', linestyle='--', alpha=0.7)
        ax3.legend(loc='best', fontsize=9)
        fig3.tight_layout()
        fig3.savefig(f".\\Result_data\\Load_Shedding_MG{MG_index}.png", dpi=300, bbox_inches='tight')
    plt.close(fig3)
