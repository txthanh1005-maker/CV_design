...
Safeguarding legacy power systems against cascading failures requires a paradigm shift from rigid, top-down architectures to decentralized microgrid communities. At the PGRE lab, my research focused on subdividing large-scale distribution networks into autonomous microgrids empowered by peer-to-peer energy trading. Because a truly equitable market must guarantee physical reliability alongside economic fairness, I integrated rigorous grid physics (AC-OPF) with distributed optimization (ADMM) across both radial and ring-connected topologies. By accounting for marginal transmission losses, this architecture reduced operational costs while strictly bounding voltage levels within the critical 0.95--1.05 pu threshold, ensuring community-level trading safeguards the wider public grid. However, while this static framework excelled under steady-state conditions, it lacked the temporal foresight to dynamically shield communities from sudden, catastrophic local failures.

True grid resilience demands dynamic fault tolerance. To address the static model's vulnerability, I layered Model Predictive Control (MPC) over the trading baseline to coordinate emergency power exchanges and backup diesel generation. During simulated localized outages, this temporal coordination empowered neighboring networks to assist a failing grid, reducing forced load shedding from 15 MWh down to just 3 MWh while protecting 100\% of critical loads. However, the computational latency of resolving these non-linear physical constraints emerged as a new critical barrier.

While my mathematical models guaranteed strict physical safety, the overhead of re-optimizing AC-OPF equations rendered classical solvers unscalable for standard tertiary dispatch intervals. As power grids integrate volatile renewable sources, this computational bottleneck threatens widespread stability. Overcoming this limitation through learning-based AI control is a societal imperative, directly motivating my decision to pursue a Ph.D.
