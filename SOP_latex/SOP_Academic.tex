...
To address 24-hour load shifting, I initially formulated a Mixed-Integer Linear Programming (MILP) model that achieved basic economic dispatch, but treating the grid as a single lumped node reduced physical power flows to purely algebraic accounting. Seeking operational granularity, I decomposed an IEEE 123-bus network to formulate a peer-to-peer active power trading framework using ADMM and Game Theory. Although this distributed architecture successfully cleared market transactions, neglecting reactive power and voltage limits rendered the economic dispatch physically infeasible. Realizing that elegant algorithms are operationally invalid if the underlying grid collapses under voltage violations, I shifted toward full AC power flow modeling to bridge optimization with rigorous grid physics.
